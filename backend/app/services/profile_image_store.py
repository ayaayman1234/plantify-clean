...
_STORE_ROOT = Path(__file__).resolve().parents[2] / "data" / "profile_images"
...
def persist_profile_image(*, image_sha256: str, image_bytes: bytes) -> None:
    _STORE_ROOT.mkdir(parents=True, exist_ok=True)
    _image_path(image_sha256).write_bytes(image_bytes)


def load_profile_image_b64(image_sha256: str | None) -> str | None:
    if not image_sha256:
        return None
